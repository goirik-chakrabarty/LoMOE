...
for image_name in tqdm(os.listdir(args.folder_name), disable='True'):
    if image_name.endswith(".png") or image_name.endswith(".jpg") or image_name.endswith(".JPEG"):
        ...
        original = Image.open(editing_prompts[image_name.split('_')[0].split('.')[0]]["image_path"])
        mask = get_bg_mask(editing_prompts[image_name.split('_')[0].split('.')[0]]["mask_path"], device, original.size)
        ...
        tgt_prompt = img_tgt_prompts[int(image_name.split('.')[0])]
