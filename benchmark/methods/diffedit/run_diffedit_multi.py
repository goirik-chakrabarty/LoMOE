    ...
    for image in tqdm(our_json):
        if not os.path.exists('{}/{}_{}.png'.format(OUT_DIR, image, str(seed))):
            ...
            for i, (mpath, fprompt, mprompt) in enumerate(zip(mask_paths, fg_prompts, mask_prompts)):
                ...
                if i == 0:
                    os.system("python diffedit.py --out_path '{}/{}/{}_{}_{}.png' --prompt '{}' --prompt_edit '{}' --origin_image '{}' --mask_path '{}' --seed {}".format(
                        OUT_DIR, image, image, str(seed), str(i), mprompt, fprompt, our_json[image]['image_path'], mpath, seed))
                else:
                    os.system("python diffedit.py --out_path '{}/{}/{}_{}_{}.png' --prompt '{}' --prompt_edit '{}' --origin_image '{}/{}/{}_{}_{}.png' --mask_path '{}' --seed {}".format(
                        OUT_DIR, image, image, str(seed), str(i), mprompt, fprompt, OUT_DIR, image, image, str(seed), str(i-1), mpath, seed))
        break
    break
