    ...
    with torch.no_grad():
        with precision_scope(device.type):
            with model.ema_scope():
                ...
                # SDEdit doesn't take a mask
                mask = np.ones([64,64])
                mask = torch.tensor(mask).cuda().half()
...
model, opt = main()
res = diffedit(model, init_image=opt.origin_image, src_prompt=opt.prompt, dst_prompt=opt.prompt_edit, seed=opt.seed)
Image.fromarray(res[0]).save(opt.out_path)

# python sdedit.py --prompt 'a cat' --prompt_edit 'a orange real dog' --origin_image "/home/prathosh/goirik/DiffEdit-stable-diffusion/inputs/recon/3.png" --out_path 'inputs/res/sddiff_out.jpg'